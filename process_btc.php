...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		var getaddress = $.ajax( '/models/get_address.php' + '?user=' + <?php echo $loggedInUser->user_id; ?> + '&type=btc' );
			  getaddress.done(function(data) {
			  	address = getAddress2(data);
			  	//clear any stacked payments
			  	var clear = $.ajax( '/models/verify_btc.php' + '?address=' + address + '&user=' + <?php echo $loggedInUser->user_id; ?> + '&fiatcurrency=<?php echo $loggedInUser->currency; ?>' );
    ...
    $('.btnkey').click(function(event){
		...
		jQuery('#dollars').keyup(function(){
			...
			var tempconversion = $.ajax( '/models/get_btc_conversion.php' + '?amount=' + amount + '&currency=<?php echo $loggedInUser->currency;?>' )
				  .done(function(data) {
				  btc = getBTC(data);
		...
		jQuery('#submit').click(function(event){ //fetch search results whenever a key is pressed
              ...
              var conversion = $.ajax( '/models/get_btc_conversion.php' + '?amount=' + amount + '&currency=<?php echo $loggedInUser->currency;?>' )
				  .done(function(data) {
			  ...
			  function autoUpdate(){
			    ...
			    var payment = $.ajax( '/models/verify_btc.php' + '?address=' + address + '&user=' + <?php echo $loggedInUser->user_id; ?> + '&fiatcurrency=<?php echo $loggedInUser->currency; ?> ');
