<?php
/*
UserCake Version: 2.0.2
http://usercake.com
*/

require_once("models/config.php");
if (!securePage($_SERVER['PHP_SELF'])){die();}
require_once("models/header.php");
require_once("models/ajax_calls.php");
?>

<div class='container container_12'>
	<div class="currency">
		<img src="images/dogecoin.png">
		<p class="supplemental">Converted Amount</p>
		<p>D<span class="converted"></span></p>
	</div>
	<div class="amount">
		<form method="post">
			<label for="dollars" class="symbol">$</label>
			<input type="number" name="dollars" id="dollars" class="dollars">
			<div class="calc">
				<div class="key">1</div>
				<div class="key">2</div>
				<div class="key">3</div>
				<div class="key">4</div>
				<div class="key">5</div>
				<div class="key">6</div>
				<div class="key">7</div>
				<div class="key">8</div>
				<div class="key">9</div>
				<div class="btn">del</div>
				<div class="key">0</div>
				<div class="key">.</div>
			</div>
			<input type="submit" id="submit" >
		</form>
	</div>
	<div class="transaction_container grid_10 push_1">
		<div class="instructions">Please send <span class="green"><span class="conversion"></span> DOGE</span> to the address below:</div>
		<div class="box">
			<div class="textaddress"></div><br>
			<div class="addressqr"></div>
		</div>
		<div class="verified offscreenbottom"></div>
	</div>
</div>
	</div>
</div>
<div id='bottom'></div>
</div>
<?php include("footer.php");?>


<script type="text/javascript">
	jQuery(document).ready(function(){

		//number pad jquery stuff
		 $('.key').click(function(){
        var numBox = jQuery('#dollars');
        if(this.innerHTML == '0'){
            if (numBox.val().length > 0) {
                numBox.val(numBox.val() + this.innerHTML);
            }
        }
        else {
            numBox.val(numBox.val() + this.innerHTML);
        }
        jQuery('#dollars').trigger("change");
        event.stopPropagation();
    });


    
    $('.btn').click(function(){
        if(this.innerHTML == 'del'){
            var numBox = jQuery('#dollars');
            if(numBox.val().length > 0){
                numBox.val( numBox.val().substring(0, numBox.val().length - 1));
            }
        }
        else{
            document.getElementById('numBox').innerHTML = '';
        }
        
        event.stopPropagation();
    });
		
		jQuery('#dollars').change(function(){
			usd = jQuery(this).val();
			var tempconversion = $.ajax( '/models/get_doge_conversion.php' + '?amount=' + usd )
				  .done(function(data) {
				  doge = getConversion2(data);
				  jQuery('.converted').text(doge);
				  });
		});

		jQuery('#submit').click(function(){ //fetch search results whenever a key is pressed
		   	  var address;
		   	  var amount;
		   	  
			  event.preventDefault();			
			  jQuery(".amount").addClass('offscreenright').fadeOut(600);
			  jQuery(".currency").addClass('offscreenleft').fadeOut(600);
			  jQuery("#dollars").blur();
			  amount = jQuery('#dollars').val();
			  
			  //fire first ajax call for conversion
              var conversion = $.ajax( '/models/get_doge_conversion.php' + '?amount=' + amount )
				  .done(function(data) {
				  //when it returns done, process it...
				    doge = getConversion2(data);
				    
				    	//then fire second call for address
					    getaddress.done(function(data) {
					    address = getAddress2(data);
					    jQuery('.transaction_container').fadeIn(500);
					    //and start checking...
					    autoUpdate();
					  });
				  })
				  .fail(function() {
				    alert( "error" );
				  })
				  
			  var getaddress = $.ajax( '/models/get_address.php' + '?user=' + <?php echo $loggedInUser->user_id; ?> + '&type=doge' );

	
			  
			  function autoUpdate(){
				//update every 5 seconds
			    var payment = $.ajax( '/models/verify.php' + '?address=' + address + '&user=' + <?php echo $loggedInUser->user_id; ?> );
				
payment.done(function(data){
					if (data >= doge){
						jQuery ('.verified').text("Payment Verified: " + data + "DOGE").fadeIn(100).removeClass('offscreenbottom');
						clearTimeout(timeoutId);
					} else if (data < doge && data > 0){
						more = doge - data;
						jQuery ('.verified').text("Insufficient Payment: " + data + "DOGE (need " + more + " more)").fadeIn(100).removeClass('offscreenbottom').addClass("insufficient");
						clearTimeout(timeoutId);
					}
				});
			    var timeoutId = setTimeout(autoUpdate, 5000);

			}
			
			
				  	
        });
        
        
    });

</script>
